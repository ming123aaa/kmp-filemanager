...
fun SettingsScreen(onBack: () -> Unit, settings: Settings? = null) {
    ...
    var serverRootPath by remember { mutableStateOf(HttpConfig.loadServerRootPath().ifEmpty { serverManager.currentConfig.rootPath }) }
    ...
    Scaffold(
        ...
        Box(
            ...
            // 服务器地址
            Text("服务器地址", style = MaterialTheme.typography.titleMedium, modifier = Modifier.padding(bottom = 12.dp))
            OutlinedTextField(
                value = serverUrl,
                    ...
                    onClick = {
                        scope.launch {
                            ...
                            try {
                                ...
                                testResult = if (result.lowercase().contains("read")) "连接成功 (只读模式)" else "连接成功"
                            } catch (e: Exception) {
            ...
            // 服务器管理
            Text("服务器管理", style = MaterialTheme.typography.titleMedium, modifier = Modifier.padding(bottom = 12.dp))
            ...
            // 下载目录
            Text("下载目录", style = MaterialTheme.typography.titleMedium, modifier = Modifier.padding(bottom = 12.dp))
            OutlinedTextField(
                value = downloadDir,
                onValueChange = { },
                label = { Text("保存位置") },
            ...
            Card(modifier = Modifier.fillMaxWidth()) {
                Column(modifier = Modifier.padding(16.dp)) {
                    Row(
                        ...
                        Text("File Manager", style = MaterialTheme.typography.bodyMedium, color = MaterialTheme.colorScheme.onSurfaceVariant)
                    }
                    Spacer(modifier = Modifier.height(8.dp))
                    Row(
                        modifier = Modifier.fillMaxWidth(),
                        ...
                        Text(getPlatform().name, style = MaterialTheme.typography.bodyMedium, color = MaterialTheme.colorScheme.onSurfaceVariant)
                    }
                }
            }
        }
